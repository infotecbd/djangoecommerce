...
#     selected_slugs = ['fruits-and-vegitables']
#     categories = Category.objects.filter(slug__in=selected_slugs)
...
#     return render(request, 'products/home.html', {'sections': sections})

def home(request):
    selected_slugs = ['fruits-and-vegitables', 'test-category']
    categories = Category.objects.filter(slug__in=selected_slugs)
    ...
    # For the "Shop by Categories" grid (all categories with product counts)
...
def fruits_veggies_section(request):
    try:
        category = Category.objects.get(slug="fruits-and-vegitables")
